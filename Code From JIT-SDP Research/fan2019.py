...
class Fan2019Dataset(object):
    __PROJECT_DATA_DIR = os.path.join('data_results', 'data_csvs2')
    __PROJECT_LIST = ['activemq', 'camel', 'derby', 'geronimo', 'hbase', 'hcommon', 'mahout', 'openjpa', 'pig', 'tuscany']
    __ATTRIBUTE_INFO = {
        'buggy_B2': 'Is this a defect-introducing changeset? obtained from B-SZZ',
        'buggy_AG': 'Is this a defect-introducing changeset? obtained from AG-SZZ',
        'buggy_MA': 'Is this a defect-introducing changeset? obtained from MA-SZZ',
        'buggy_RA': 'Is this a defect-introducing changeset? obtained from RA-SZZ, considerd most accurate',
    ...
    __LABEL_MAPPER = {'clean': 0, 'buggy': 1}
    __LABEL_LIST = ['buggy_B2', 'buggy_AG', 'buggy_MA', 'buggy_RA']

    def __init__(self, data_dir:str):
        if not os.path.exists(data_dir):
            raise FileNotFoundError('Fan-2019 Dataset not found at {}'.format(data_dir))
        
        input_dir = os.path.join(data_dir, self.__PROJECT_DATA_DIR)            
        if not os.path.exists(input_dir):
            raise FileNotFoundError('Complete Fan-2019 Dataset not found at {}'.format(data_dir))
    ...
    @classmethod
    def list_features(self, type='as_is') -> Iterable:
        if type == 'as_is':
    ...
    def get_as_is_data(self, project:str) -> pd.DataFrame:
        ...
        for label in self.__LABEL_LIST:
            df.loc[:, label] = df[label].map(self.__LABEL_MAPPER)
        return df   
